...
1. 在右侧的“**选择用户**”浮出控件页上，选择后续步骤中要用于执行电子数据展示调查的用户，然后选择“**选择**”。

    >**备注**：确保选择要查看数据和导出搜索结果的用户。
...
1. 在 Microsoft Purview 中，选择“**解决方案**” > “**电子数据展示”**。

   这将直接转到**电子数据展示（高级版）** 的**经典**体验。

1. 在“**电子数据展示（高级版）**”页上，选择“**案例**”选项卡，然后选择“**+ 创建案例**”。
...
1. 在“**定义搜索查询**”页上，使用查询生成器生成搜索以查找与案例相关的内容：
   ...
   - 使用 **AND** 运算符搜索“**等于**”`legal` 的“**关键字**”。
    ...
    >**备注**：创建搜索查询后，可能需要删除用于**选择筛选器**的选项，以查找等于**法律信息**的关键字。 如果此选项存在，则会阻止在后续步骤中添加子组。
    >![显示电子数据展示高级版中查询生成器的屏幕截图。](./Media/ediscovery-subgroup-bug.png)
